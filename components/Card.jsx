...
function Card() {
  ...
  const fetchMeta = async () => {
    setLoading(true);
    axios
      .get(`/api/fetchMeta/?url=${urlRef.current.value}`)
      .then((res) => {
        setMetaData(res);
          ...
          <div className=' flex justify-end '>
            <div className=' flex items-center relative justify-evenly mr-[20px] mt-[20px] mb-[10px]  h-[30px] w-[70px] rounded-2xl '>
              <FontAwesomeIcon
                icon={faMoon}
                onClick={switchTheme}
                className={` text-[20px] p-[5px] hover:text-gray-600 transition cursor-pointer duration-450 ease-in-out ${
                  !dark ? ' ' : 'bg-yellow-400'
                }   rounded-full  `}
              />
              <FontAwesomeIcon
                icon={faSun}
                onClick={switchTheme}
                className={` text-[20px]  p-[5px] hover:text-gray-600 cursor-pointer transition duration-450 ease-in-out ${
                  dark ? ' ' : 'bg-yellow-400'
                }  rounded-full  `}
              />
            </div>
            ...
            <div className='flex flex-col items-center max-w-[550px]  '>
              <div className=' text-primary dark:text-gray-300 w-full flex flex-col items-center '>
                ...
                <motion.h1
                  ...
                  initial={{ y: 10, opacity: 0 }}
                  animate={{ y: 0, opacity: 1 }}
                  className=' uppercase font-semibold md:text-[96px] text-[80px] leading-none '
                >
                  Carding
                </motion.h1>
              </div>
              ...
              >
                ...
                <div className=' flex items-center cursor-pointer  uppercase bg-secondary px-2 rounded-md text-white transition duration-350 ease-in-out hover:bg-green-600 '>
                  <FontAwesomeIcon
                    ...
                    className=' mr-[5px]  font-normal text-[14px] '
                  />
                  ...
                  <div className=' flex flex-col  max-w-[550px] mb-8 drop-shadow-sm bg-gray-100 border-[1px] dark:bg-gray-700 border-green-200 p-[10px] rounded-md  '>
                    <div className=' flex justify-end mb-[15px] mt-[5px] '>
                      {metaData.data.response.image ? (
                        <div className=' flex items-center uppercase text-white bg-gray-800 py-[5px] px-[8px] rounded-md transition duration-350 ease-in-out hover:bg-slate-600 hover:text-white  '>
                          <a
                            href={`/api/download?url=${metaData.data.response.image.url}`}
                          >
                            Download
                          </a>
                          <FontAwesomeIcon
                    ...
                    <div className=' rounded-md overflow-hidden '>
                      {metaData.data.response.image ? (
                        <div>
                          <img
                            src={metaData.data.response.image.url}
                            className=' w-full '
                            alt=''
                          />
                        </div>
                    ...
                    <div className=' mt-[10px] '>
                      <h1 className=' text-[20px] font-semibold text-gray-00 break-words dark:text-gray-400 mb-[10px] '>
                        {metaData.data.response.title}
                      </h1>

                      <p className=' text-gray-500 text-[18px]   mb-[5px] break-words '>
                        {metaData.data.response.description &&
                          metaData.data.response.description}
                      </p>
                      ...
                      <div className=' flex items-center text-gray-600 dark:text-gray-400 '>
                        ...
                        />
                        <h2 className='  text-[18px]  antialiased font-semibold '>
